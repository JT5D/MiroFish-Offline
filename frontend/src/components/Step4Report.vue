...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Interview subject selection reasoning\n([\s\S]*?)(?=\n---\n|\n### Interview transcripts)/)
    if (reasonMatch) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          if (interview.redditAnswer && interview.redditAnswer !== '(No response received from this platform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No response received from this platform)') {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (new backend format)

...
2. Plan the outline structure first, then generate content section by section
...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (content only, does not represent the entire section being complete)"""
        self.log(
...
This is not LLM simulation, but calls the actual interview interface to get the simulation Agents' original responses.
...
4. Calls /api/simulation/interview/batch interface for real dual-platform interviews
...
- Want the report to be more vivid, containing "interview transcripts"
...
[Core Concept]
We have built a simulated world and injected a specific "simulation requirement" as a variable. The evolution results of the simulated world represent a prediction of what might happen in the future. What you are observing is not "experimental data", but a "rehearsal of the future".
...
- The statements and behaviors of Agents in the simulated world are predictions of future group behavior
...
and the behaviors and interactions of Agents in the simulation are predictions of future group behavior.
...
3. [Language Consistency - Report Must Be Written in English]
   ...
   - When quoting content from tools, ensure it is presented in clear English
   - Maintain the original meaning, ensuring natural and smooth expression
...
[Correct Example]
```
This section analyzes the public opinion transmission dynamics of the event. Through in-depth analysis of simulation data, we found...

**Initial Ignition Phase**

Weibo served as the primary scene of public opinion, taking on the core function of initial information release:
...
[Tool Usage Suggestions - Mix different tools, don't only use one type]
...
Each response can only do one of the following two things (cannot do both simultaneously):
...
3. Use Markdown format (but do NOT use headings):
   - Use **bold text** to mark key points (as a replacement for sub-headings)
...
4. [Quote Format Guidelines - Must Be Separate Paragraphs]
   ...
   > "The university's response pattern appeared rigid and sluggish in the rapidly changing social media environment."
...
3. Please mix different tools, don't only use one type
...
REACT_TOOL_LIMIT_MSG = (
    "Tool call limit reached ({tool_calls_count}/{max_tool_calls}), cannot call any more tools. "
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        for iteration in range(max_iterations):
            ...
            logger.info(f"Section {section.title} no 'Final Answer:' prefix detected, directly accepting LLM output as final content (tool calls: {tool_calls_count})")
    ...
    def generate_report(
        ...
        Each section is saved to a file immediately after generation, no need to wait for the entire report to complete.
        File structure:
        reports/{report_id}/
        ...
        try:
            ...
            # Phase 2: Generate content section by section (save each section)
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line number (for incremental fetching, 0 means from the beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check if it is a duplicate of the section title (within the first 5 lines)
                if i < 5:
            ...
            # If the previous line was a skipped title, and current line is empty, also skip
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        if not os.path.exists(path):
            # Backward compatible: check files stored directly in the reports directory

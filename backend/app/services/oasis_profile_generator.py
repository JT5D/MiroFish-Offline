...
class OasisProfileGenerator:
    ...
    Converts entities from the graph into Agent Profiles required by OASIS simulation

    Optimization features:
    1. Calls graph retrieval functions for richer context
    ...
    # Group/institution type entities (require generating representative account personas)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_graph_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use GraphStorage hybrid search to get rich information related to the entity
        ...
        try:
            ...
            logger.info(f"Graph hybrid retrieval complete: {entity_name}, retrieved {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 4. Use graph hybrid retrieval for richer information
        ...
        if graph_results.get("facts"):
            ...
            if new_facts:
                context_parts.append("### Facts Retrieved from Graph\n" + "\n".join(f"- {f}" for f in new_facts[:15]))

        if graph_results.get("node_summaries"):
            context_parts.append("### Related Nodes Retrieved from Graph\n" + "\n".join(f"- {s}" for s in graph_results["node_summaries"][:10]))
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, restoring the existing real-world situation as much as possible. You must return valid JSON format, and all string values must not contain unescaped newlines. Use English for all output content."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, restoring the existing real-world situation as much as possible.
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stances and opinions (attitude toward topics, content that might provoke or move them)
   - Unique characteristics (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of the persona, should describe the individual's connection to events, and their existing actions and reactions in the events)
...
6. country: Country (in English, e.g., "United States")
...
- Use English for all fields (gender must be male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institution/group entity, restoring the existing real-world situation as much as possible.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   ...
   - Special notes (group portrait it represents, operational habits)
   - Institutional memory (important part of the institutional persona, should describe the institution's connection to events, and its existing actions and reactions in the events)

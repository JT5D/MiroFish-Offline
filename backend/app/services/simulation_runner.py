...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if enable_graph_memory_update:
            ...
            try:
                if not storage:
                    raise ValueError("storage (GraphStorage) is required when graph memory update is enabled")
        ...
        try:
            ...
            # This fixes issues where third-party libraries (like OASIS) don't specify encoding when reading files
            env = os.environ.copy()
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            # If graph memory update is enabled, send activity to the graph
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines which platforms are enabled by checking whether corresponding actions.jsonl files exist
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer platform from the record, otherwise use default platform
    ...
    def cleanup_all_simulations(cls):
        ...
        # First stop all graph memory updaters (stop_all internally prints logs)
        try:
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in the reloader child process (the process that actually runs the app)
        # WERKZEUG_RUN_MAIN=true indicates the reloader child process
        # In non-debug mode, this environment variable doesn't exist, so cleanup should also be registered
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: Interview on both platforms in dual-platform mode, returning integrated results
    ...
    def close_simulation_env(
        ...
        Send a close environment command to the simulation, making it gracefully exit the command waiting mode
    ...
    def get_interview_history(
        ...
        Args:
            ...
            agent_id: Specified Agent ID (optional, get only that Agent's history)

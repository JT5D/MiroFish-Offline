...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert the activity into a natural language text description

        Uses natural language description format so the NER extractor can extract entities and relationships from it
        """
        action_descriptions = {
...
class GraphMemoryUpdater:
    ...
    Monitors the simulation's actions log file and updates new agent activities to the graph in real time.
    Groups activities by platform and sends them in batches to the graph after accumulating BATCH_SIZE activities.
    """
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        """
        Send activities to the graph in batches (merged into one text, triggering NER via add_text)
        """
        if not activities:
            return

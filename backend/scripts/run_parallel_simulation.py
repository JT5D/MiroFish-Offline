...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None,
        ...
        """
        Wrapper for open() function, defaults to UTF-8 encoding for text mode
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview on both platforms simultaneously, return integrated results
...
def fetch_new_actions_from_db(
    ...
    try:
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Simplify action_args, only keep key fields (preserve full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    """
    Supplement context information for actions (post content, usernames, etc.)
    ...
    try:
        ...
        # Create comment: supplement information about the commented post
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum simulation rounds (optional, for truncating excessively long simulations)
    ...
    # Get Agent real name mapping from config file (using entity_name instead of default Agent_X)
    ...
    for round_num in range(total_rounds):
        ...
        if not active_agents:
            # Log round end even when there are no active agents (actions_count=0)
            if action_logger:
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Maximum simulation rounds (optional, for truncating excessively long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environment immediately after simulation completes, do not enter command waiting mode'
    )
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (give the loop a chance to clean up resources)
            if _shutdown_event:
                _shutdown_event.set()

        # Don't call sys.exit() directly, let the asyncio loop exit normally and clean up resources
        # Only force exit if signal is received again
        else:
            print("Force exiting...")
            sys.exit(1)
